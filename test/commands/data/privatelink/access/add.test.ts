...
describe('data:privatelink:access:add', () => {
  test
    .nock('https://postgres-api.heroku.com', api => api
      .put('/private-link/v0/databases/postgres-123/allowlisted_accounts', {allowlisted_accounts: ['123456789012:root']})
      .reply(200, {})
    )
    ...
    .it('adds an allowed account', ctx => {
      expect(ctx.stderr).to.contain('Adding account')
      expect(ctx.stderr).to.contain('... done')
    })

  test
    .nock('https://postgres-api.heroku.com', api => api
      .put('/private-link/v0/databases/postgres-123/allowlisted_accounts', {allowlisted_accounts: ['123456789012:resource1', '123456789012:resource2']})
      .reply(200, {})
    )
    .nock('https://api.heroku.com', api => api
      .post('/actions/addons/resolve')
