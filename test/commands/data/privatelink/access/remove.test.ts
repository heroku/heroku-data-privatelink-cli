...
describe('data:privatelink:access:remove', () => {
  test
    .nock('https://postgres-api.heroku.com', api => api
      .patch('/private-link/v0/databases/postgres-123/allowlisted_accounts', {allowlisted_accounts: ['123456789012:root']})
      .reply(200, {})
    )
    ...
    .it('removes an allowed account', ctx => {
      expect(ctx.stderr).to.contain('Removing account')
      expect(ctx.stderr).to.contain('... done')
    })

  test
    .nock('https://postgres-api.heroku.com', api => api
      .patch('/private-link/v0/databases/postgres-123/allowlisted_accounts', {allowlisted_accounts: ['123456789012:resource1', '123456789012:resource2']})
      .reply(200, {})
    )
    .nock('https://api.heroku.com', api => api
      .post('/actions/addons/resolve')
